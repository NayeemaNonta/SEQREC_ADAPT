#!/usr/bin/env python3
"""
adaptation/context_steering/sweep.py

Grid search over context-gate adapter hyperparameters.
...
python adaptation/context_steering/sweep.py \
  ...
  --base_outdir  results/sweep_context_gate \
  --device cuda
"""
...
CSV_COLUMNS = (
    ["run_id", "bottleneck_dim", "lr", "epochs", "best_eval_loss"]
...
def main():
    ...
    for run_id, cfg in enumerate(configs, 1):
        name    = f"run{run_id:03d}_bd{cfg['bottleneck_dim']}_lr{cfg['lr']}_ep{cfg['epochs']}"
