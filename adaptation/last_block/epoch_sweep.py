#!/usr/bin/env python3
"""
adaptation/last_block/epoch_sweep.py

Sweep over epochs [10, 20, 50] with all other HPs fixed to the best config.

Best config (fixed): lr=5e-4, include_last_layernorm=True, weight_decay=1e-4
Swept: epochs in [10, 20, 50]

Usage:
python adaptation/last_block/epoch_sweep.py \
  ...
  --base_outdir results/epoch_sweep_last_block_contiguous --device cuda
"""
...
def main():
    ...
    print(f"[epoch_sweep] last_block | epochs={epoch_list} | fixed config: {BEST}")
    ...
    for run_id, ep in enumerate(epoch_list, 1):
        ...
        row = build_sweep_row(name, {"epochs": ep}, summary, key_adapted="finetuned_metrics")
